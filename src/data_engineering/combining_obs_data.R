...
met_eireann = read.csv("met_eireann_data_mintp.csv") %>% as_tibble %>%
  dplyr::select(loc_id,date,mintp,Station,Lat,Long)
met_eireann[met_eireann$Station == "DUBLIN (MERRION SQUARE)",]$loc_id = 3923 # --- we have 2 "DUBLIN (MERRION SQUARE)" sites in me_sites


ni = read.csv("NI_mintp_data.csv") %>% 
  as_tibble %>%
  dplyr::select(loc_id = id,date,mintp,Station,Lat,Long)

# ----- combine data
obs_data = rbind(met_eireann,ni)
...
obs_data %>%
  ...
  as_tibble() %>%
  unique() %>%
  write_csv("obs_all_data_mintp.csv")
