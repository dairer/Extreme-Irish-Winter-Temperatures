...
file.loc = "~/data/raw/clim_tn/"
file.save = "~/data/processed/mintp_r12i1p1-ICHEC-EC-EARTH-(Ireland)CLMcom-CLM-CCLM4-8-17(EU).csv"
...
for(f in paste0(file.loc, my_files)){
  ...
  print(f)
  my_dat = raster::brick(f, varname = 'tasmin') # read in netcdf file
  ...
  all_data %>%
    ...
    dplyr::select(-id) %>%
    tidyr::pivot_longer(c(-Long, -Lat), names_to = "date", values_to = "mintp") %>% # Making each row an observation
    mutate(date = date %>% lubridate::ymd()) %>%
    mutate(mintp = mintp - 273.15) %>%
    dplyr::mutate(id = group_indices(., Long, Lat)) %>%
